...
public class Byte512VectorTests extends AbstractVectorTest {
    ...
    @Test(dataProvider = "byteUnaryOpProvider")
    static void ADDReduceLongByte512VectorTests(IntFunction<byte[]> fa) {
        byte[] a = fa.apply(SPECIES.length());
    ...
    @Test(dataProvider = "byteUnaryOpMaskProvider")
    static void ADDReduceLongByte512VectorTestsMasked(IntFunction<byte[]> fa, IntFunction<boolean[]> fm) {
        byte[] a = fa.apply(SPECIES.length());
        long[] r = lfr.apply(SPECIES.length());
        boolean[] mask = fm.apply(SPECIES.length());
        VectorMask<Byte> vmask = VectorMask.fromArray(SPECIES, mask, 0);
